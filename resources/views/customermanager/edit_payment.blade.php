                    ...
                    <div class="panel-body">
                        ...
                        <form action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" required name="customer_id">
                                    ...
                                    @foreach($customers as $customer)
                                        <option {{ $payment_edit->customer_id == $customer->id ? 'selected' : "" }} value="{{ $customer->id }}">{{ $customer->firstname }} {{ $customer->lastname }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" required name="payment_method" id="payment_method">
                                    ...
                                    @foreach($payments as $payment)
                                        <option {{ $payment_edit->payment_method_tables()->first()->payment_method_id ==  $payment->id ? 'selected' : ""  }}  data-label="{{ strtolower( $payment->name) }}"  value="{{  $payment->id }}">{{  $payment->name }}</option>
                                    @endforeach
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" value="{{ $payment_edit->total_paid }}" required  placeholder="Amount Paid" class="form-control" name="amount"/>
                            </div>

                            <div class="form-group">
                                <label>Payment Date</label>
                                <input type="text"  required class="form-control datepicker js-datepicker" value="{{ date('Y-m-d', strtotime($payment_edit->payment_date)) }}" data-min-view="2" data-date-format="yyyy-mm-dd" style="background-color: #FFF; color: #000;"   name="payment_date" placeholder="Payment Date"/>
                            </div>
                            ...
                            <button type="submit" class="btn btn-primary text-center"><i class="fa fa-save"></i> Update Payment</button>
